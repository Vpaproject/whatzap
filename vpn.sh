...
systemctl enable --now openvpn-server@server-tcp-1103
systemctl enable --now openvpn-server@server-udp-25222
...
cat /etc/openvpn/server/tls-auth.key >> /etc/openvpn/UDP.ovpn
echo '</tls-auth>' >> /etc/openvpn/UDP.ovpn
